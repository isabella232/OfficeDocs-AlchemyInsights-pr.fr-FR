---
title: 125 obtention d’une erreur de licence non valide dans le site web d’Outlook ?
ms.author: daeite
author: daeite
manager: jackiesm
ms.date: 4/9/2018
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Priority
ms.collection: Adm_O365
ms.custom: Adm_O365
ms.assetid: 6d9947d9-6c92-4ada-b655-8ab2a0c2b66d
ms.openlocfilehash: 39ef2ce55f4884ac789445c7446e15ba732bbd43
ms.sourcegitcommit: d6ea5e9458a2b8ceaab3ac4bd483e1130b9a398a
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 01/15/2019
ms.locfileid: "28288117"
---
# <a name="getting-an-invalid-license-error-in-outlook-on-the-web"></a>Obtention d’une erreur de licence non valide dans le site web d’Outlook ?

Si vous utilisez Outlook sur le web et vous obtenez une erreur **d’un problème** contient **X-OWA-erreur : Microsoft.Exchange.Data.Storage.InvalidLicenseException**, votre licence Exchange Online n’est pas correctement attribué ou qu’il a récemment a expiré. Votre administrateur peut attribuer une licence à vous en suivant ces étapes :
  
1. Ouvrez le [Centre d’administration Office 365](https://portal.office.com/adminportal/home#/homepage) et sous **utilisateurs actifs**, sélectionnez **Modifier un utilisateur**.
    
2. Dans la page **Modifier un utilisateur** qui s’ouvre, sélectionnez l’utilisateur. Dans la page de propriétés d’utilisateur qui s’ouvre, cliquez sur **Modifier** pour les **licences de produit**.
    
3. Dans la page **licences de produits** qui s’ouvre, sélectionnez la valeur de **l’emplacement** appropriée et attribuer une licence qui contient Exchange Online (développer la licence pour afficher ses détails). Lorsque vous avez terminé, cliquez sur **Enregistrer**.
    

